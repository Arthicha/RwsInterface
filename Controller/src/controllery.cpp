...
int main (int argc, char** argv)
{
    ...
    static const std::string WC_FILE = "../../Project_WorkCell/Scene.wc.xml";
    const WorkCell::Ptr wc = WorkCellLoader::Factory::load (WC_FILE);
    if (wc.isNull ())
        RW_THROW ("WorkCell could not be loaded.");
    Frame* const camera = wc->findFrame ("Camera_Right");
    if (camera == nullptr)
    ...
    RWS_START (app)
    {
        ...
        framegrabber->grab(camera, state);
        const rw::sensor::Image* rw_image = &(framegrabber->getImage());
